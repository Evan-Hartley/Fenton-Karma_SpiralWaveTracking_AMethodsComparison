# Fenton-Karma Spiral Wave Tracking: A Methods Comparison

Compare and Contrast the Contour and Phase Identification Methods for Spiral Wave Identification using the Fenton-Karma Model
...
```
nvcc main.cpp CUDAfiles/ComputeFK.cu PreCompiledHeader/FKpch.cpp -o fkModel.exe
```
**-OR-**
...
The `fkModel.exe` file will then be in a subfolder entitled `build/run/`

## Running the Code

This code is designed to be run on the Georgia Tech PACE Phoenix cluster, but can be run on any computer with a NVIDIA GPU and CUDA installed. To run locally, either open the constructed `fkModel.exe` file or use your terminal to run the program by navigating to the `fkModel.exe` and run either the `start fkModel.exe` command (Windows) or the `wine fkModel.exe` command (Linux and macOS).
